...
if [ -n "$target_date" ]; then
  ...
  print_info "すべての日付を処理"
  
  if [ "$DEBUG" = "true" ]; then
    print_debug "コマンド実行: PROJECT_ROOT=${PROJECT_ROOT} npx tsx --trace-warnings ${SCRIPTS_DIR}/update-journal-entries.ts"
    PROJECT_ROOT="${PROJECT_ROOT}" npx tsx --trace-warnings "${SCRIPTS_DIR}/update-journal-entries.ts"
  else
    PROJECT_ROOT="${PROJECT_ROOT}" npx tsx "${SCRIPTS_DIR}/update-journal-entries.ts"
  fi
fi
...
if [ -n "$github_user" ]; then
  ...
  PROJECT_ROOT="${PROJECT_ROOT}" npx tsx "${SCRIPTS_DIR}/update-journal-commits.ts" --user "$github_user"
elif [ -n "$REPO_OWNER" ] && [ -n "$REPO_NAME" ]; then
  print_info "リポジトリのコミット情報を取得: $REPO_OWNER/$REPO_NAME"
  PROJECT_ROOT="${PROJECT_ROOT}" npx tsx "${SCRIPTS_DIR}/update-journal-commits.ts" --owner "$REPO_OWNER" --repo "$REPO_NAME"
else
  print_info "デフォルトユーザーのコミット情報を取得: otomatty"
  PROJECT_ROOT="${PROJECT_ROOT}" npx tsx "${SCRIPTS_DIR}/update-journal-commits.ts" --user "otomatty"
fi
