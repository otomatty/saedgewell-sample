...
type WhitelistValue = (string | RegExp)[];
type Whitelist = {
	[key: string]: WhitelistValue;
};
...
function checkEnvFiles(rootPath: string) {
 ...
 * @returns {boolean}
 */
function isValueWhitelisted(key: string, value: string): boolean {
	if (!(key in whitelist)) {
		return false;
	}
