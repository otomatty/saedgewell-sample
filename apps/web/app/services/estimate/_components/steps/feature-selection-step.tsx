...
export function FeatureSelectionStep() {
  ...
  const generateFeaturesList = useCallback(async () => {
    console.log('generateFeaturesList called. formData:', formData);
  ...
  useEffect(() => {
    console.log(
      'useEffect triggered. proposedFeatures.length:',
      proposedFeatures.length,
      'isInitialMount:',
      isInitialMount.current
    );
    if (isInitialMount.current && proposedFeatures.length === 0) {
      console.log('Calling generateFeaturesList from useEffect');
