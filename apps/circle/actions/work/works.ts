...
import type { Work, WorksFilter, WorkWithRelations } from '@kit/types';
...
export async function getWorkSlugsForBuild() {
  try {
    ...
    if (error) {
      ...
      throw new Error(`Failed to fetch work slugs: ${error.message}`);
    }

    if (!works || works.length === 0) {
      // 空配列を返す（作品が見つからない場合でもエラーにしない）
      return [];
    }

    return works;
  } catch (err) {
    console.error('Error in getWorkSlugsForBuild:', err);
    throw new Error(
      `Failed to fetch work slugs: ${err instanceof Error ? err.message : String(err)}`
    );
  }
}
 ...
 * ステータスに基づいて実績を取得する
 */
export async function getWorksByStatus(status: Work['status']) {
  const supabase = await getSupabaseServerClient();
  ...
  if (error) {
    throw new Error(`実績の取得に失敗しました: ${error.message}`);
  }
...
export async function getFeaturedWorks() {
  return getWorksByStatus('featured');
}
...
export async function getPublishedWorks() {
  return getWorksByStatus('published');
}
