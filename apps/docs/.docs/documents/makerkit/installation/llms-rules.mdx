---
title: 'CursorとWindsurfのためのLLMルール'
description: 'LLMのための最適なコンテキストを生成するために、CursorとWindsurfでこれらのルールを使用してください'
icon: 'brain'
order: 14
---

# CursorとWindsurfのためのLLMルール

以下の内容は、LLMのための最適なコンテキストを生成するためのCursorとCodeium Windsurfのためのガイドラインです。これは、Windsurfなどの他のコーディングツールでもうまく機能するはずです。

Cursor: プロジェクトのルートに`.cursorrules`ファイルを作成し、以下のルールを貼り付けてください。
Windsurf: プロジェクトのルートに`.windsurfrules`ファイルを作成し、このページの下部にあるルールを貼り付けてください。

注意事項: これらのツールは優れていますが、完璧ではありません。常に結果を二重チェックし、それらが関連性があり有用であることを確認してください。

## MakerkitのためのCursorルール

Cursorには指定された制限がないため、以下の内容を使用することをお勧めします：
...
## MakerkitのためのWindsurfルール

Windsurfには、1ファイルあたり6000文字という制限があるため、以下の内容を使用することをお勧めします：
...
私たちの小さなスクリプトとオープンソースのMakerkitドキュメントを使用して、一連のMarkdownファイルを生成することができます。

これらのMarkdownファイルを、Cursor、ChatGPT、Claudeなどのあなたのためのナレッジベースとして使用できます。
...
より正確なコンテキストを確保するために、常に興味のあるキットに対して生成をスコープしてください。例えば、Remix Supabase Kitのmarkdownファイルを生成するには、次のコマンドを実行します：

```bash
node index.js kits/remix-supabase-turbo
```

また、次のコマンドを実行することで、コースに対しても同様のことができます：

```bash
node index.js courses
```

これにより、distフォルダにmarkdownファイルが生成されます。

スクリプトに2番目の引数を渡すことで、ファイルごとの単語数を設定することもできます。例：

```bash
node index.js kits 4000
```

デフォルトでは、ファイルごとに5000単語が生成されます。
